...
function setImage(side, pathBase, codec, setText) {
	...
	var filename = getSelValue(select[side], 'fname');
    var path = urlFolder.concat(pathBase, '/', filename, '.', codec);
...
function setSide(side) {
	...
	// scene/crop folder
    var pathBase = getSelValue(select.file, 'value');

    if (false) { //(pathBase != 'Original') {
